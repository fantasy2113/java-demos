...
public class AggregationDemo {
	...
	public static void main(String[] args) {
		...
		persons.stream().max((p1, p2) -> p2.getId() - p1.getId()).ifPresent(System.out::println);

		// 10 zufällige Zahlen -> Die größte von Diesen
		IntStream.rangeClosed(1, 10).map((i) -> Double.valueOf(Math.random() * 100).intValue()).max() // .min
		...
		persons.stream().collect(Collectors.groupingBy(Person::getCountry, Collectors.counting()))
		...
		persons.stream().collect(Collectors.groupingBy(Person::getCountry, Collectors.counting())).entrySet().stream()
				...
				.max(Comparator.comparingInt(e -> e.getValue().intValue())).ifPresent(
						...
						biggestCountry -> persons.stream()
								.filter(person -> person.getCountry().equals(biggestCountry.getKey()))
