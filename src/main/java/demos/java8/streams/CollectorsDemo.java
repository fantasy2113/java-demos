...
public class CollectorsDemo {
    ...
    public static void main(String[] args) {
        ...
        Person[] personArray = persons.stream().filter(p -> p.getFirstname().startsWith("M")).toArray(Person[]::new);
        System.out.println(Arrays.toString(personArray));

        // Erzeugen eines Collection Objektes
        List<Person> personList = persons.stream().filter(p -> p.getFirstname().startsWith("C"))
        ...
        persons.stream().filter(p -> p.getFirstname().startsWith("H")).forEach(System.out::println);
